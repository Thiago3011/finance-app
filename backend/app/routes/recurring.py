...
class RecurringCreate(BaseModel):
    name: str
    amount: float
    due_day: int               # ✅ item 1: sempre obrigatório
...
def list_recurring(db: Session = Depends(get_db)):
    ...
    variable = db.query(Recurring).filter(Recurring.is_variable == True).order_by(Recurring.due_day).all()
...
def create_recurring(data: RecurringCreate, db: Session = Depends(get_db)):
    ...
    item = Recurring(
        name=data.name, amount=data.amount, due_day=data.due_day,
        category_id=data.category_id, icon=data.icon,
        active=data.active, is_variable=data.is_variable,
    )
...
def update_recurring(id: int, data: RecurringCreate, db: Session = Depends(get_db)):
    ...
    if not item: raise HTTPException(404, "Não encontrado")
    if not 1 <= data.due_day <= 31:
        raise HTTPException(400, "Dia de vencimento deve ser entre 1 e 31")
    item.name = data.name; item.amount = data.amount; item.due_day = data.due_day
    item.category_id = data.category_id; item.icon = data.icon
    item.active = data.active; item.is_variable = data.is_variable
...
def recurring_for_month(year: int, month: int, db: Session = Depends(get_db)):
    ...
    for item in items:
        ...
        result.append({
            ...
            "due_day": item.due_day, "icon": item.icon,
            "is_variable": item.is_variable, "category_id": item.category_id,
            "paid": paid, "paid_amount": paid_amount, "transaction_id": tx_id,
            "status": status,
...
def pay_recurring(id: int, body: dict, db: Session = Depends(get_db)):
    ...
    item = db.query(Recurring).filter(Recurring.id == id).first()
    if not item: raise HTTPException(404, "Conta fixa não encontrada")

    year = body.get("year", date.today().year)
    month = body.get("month", date.today().month)
    amount = body.get("amount", item.amount)
    account_id = body.get("account_id")
    fine = body.get("fine", 0) or 0  # ✅ item 4: multa por atraso

    if not account_id: raise HTTPException(400, "Informe a conta (account_id)")

    # Remove lançamento anterior se existir
    existing = db.query(Transaction).filter(
        Transaction.description.like(f"[FIXA:{item.id}]%"), Transaction.type == "expense",
    ).all()
    for t in existing:
    ...
    return {"ok": True, "transaction_id": tx.id, "amount": total_paid, "fine": fine}
