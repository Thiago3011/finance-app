...
export async function markTransactionPaid(id: number, paid: boolean) {
  const res = await fetch(`${API_URL}/transactions/${id}/paid`, {
    method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ paid })
  })
  return res.json()
}
...
export async function createInstallmentCustom(data: any) {
  const res = await fetch(`${API_URL}/installments/custom`, {
    method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data)
  })
  return res.json()
}
...
export async function deleteInstallment(id: number) {
  await fetch(`${API_URL}/installments/${id}`, { method: "DELETE" })
}
