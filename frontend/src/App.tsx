...
.nav-logo{font-family:'Bricolage Grotesque',sans-serif;font-size:20px;font-weight:800;background:linear-gradient(130deg,var(--acc),var(--acc2));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;padding:15px 16px 15px 8px;margin-right:4px;border-right:1px solid var(--b1)}
.nav-tab{background:none;border:none;border-bottom:2px solid transparent;color:var(--muted2);cursor:pointer;font-family:'Plus Jakarta Sans',sans-serif;font-size:13px;font-weight:500;padding:15px 14px;transition:all .18s;white-space:nowrap}
...
.sc{background:var(--s1);border:1px solid var(--b1);border-radius:var(--r);padding:18px 20px;position:relative;overflow:hidden;transition:border-color .2s,transform .15s;cursor:default}
...
.be{background:none;border:none;cursor:pointer;color:var(--muted);font-size:13px;padding:4px 6px;border-radius:6px;transition:all .15s}.be:hover{color:var(--acc);background:rgba(124,109,250,.1)}
...
.b-y{background:rgba(251,191,36,.12);color:var(--yellow);font-size:10px}
...
.amt-input{width:90px;padding:4px 7px;font-size:12px;border-radius:7px}
...
.rec-pay-row{display:flex;align-items:center;justify-content:space-between;background:var(--s2);border:1px solid var(--b2);border-radius:10px;padding:10px 13px;margin-bottom:6px;transition:border-color .2s}
.rec-pay-row.paid-border{border-color:rgba(74,222,128,.3)}
.rec-pay-row:hover{border-color:var(--b3)}
...
.custom-num{font-size:11px;font-weight:700;color:var(--muted2);min-width:22px}

.acc-bar-item{background:var(--s2);border:1px solid var(--b2);border-radius:10px;padding:12px 14px;margin-bottom:7px}
...
@media(max-width:1024px){.sum-grid{grid-template-columns:1fr 1fr}.g2,.g2b{grid-template-columns:1fr}}
        ...
        <form onSubmit={save}>
          ...
          <div style={{background:"var(--s2)",border:"1px solid var(--b2)",borderRadius:10,padding:"12px 14px",marginBottom:12}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:isVar?10:0}}>
              ...
              <label htmlFor="var-chk" style={{fontSize:13,cursor:"pointer",fontWeight:500}}>Conta variável (ex: combustível, mercado)</label>
            </div>
            {isVar&&<div style={{fontSize:12,color:"var(--muted2)",lineHeight:1.5}}>Você poderá atualizar o valor e informar o que pagou todo mês.</div>}
          </div>
          <div className="fr" style={{marginBottom:12}}>
...
function EditInstModal({ initial, onSave, onClose }: { initial:InstallmentSummary; onSave:(d:any)=>void; onClose:()=>void }) {
  ...
  const [dt,setDt] = useState(initial.debt_type)
        ...
        <form onSubmit={e=>{e.preventDefault();onSave({description:desc,debt_type:dt,total_amount:Number(total)})}}>
          ...
          <div style={{marginBottom:12}}>
            ...
            <div className="dt-btns" style={{marginTop:6}}>
              {Object.entries(DEBT_CFG).map(([k,cfg])=><button key={k} type="button" className={`dt-btn ${dt===k?`s-${k}`:""}`} onClick={()=>setDt(k)}>{cfg.icon} {cfg.label}</button>)}
            </div>
          </div>
...
function ManageModal({ title, items, itemType, onCreate, onUpdate, onDelete, onClose }: {
  ...
  const [newName,setNewName] = useState(""); const [newType,setNewType] = useState("expense")
  const [editId,setEditId] = useState<number|null>(null); const [editName,setEditName] = useState(""); const [editType,setEditType] = useState("expense")
  return (
    <div className="mbg" onClick={e=>{if(e.target===e.currentTarget)onClose()}}>
      <div className="modal">
        ...
        <div style={{background:"var(--s2)",border:"1px solid var(--b2)",borderRadius:10,padding:12,marginBottom:14}}>
          <div className="fr" style={{marginBottom:8}}>
            ...
            {itemType==="category"&&<select value={newType} onChange={e=>setNewType(e.target.value)} style={{maxWidth:130}}><option value="expense">Despesa</option><option value="income">Receita</option></select>}
          </div>
          <button className="btn bp bsm" onClick={()=>{if(!newName.trim())return;onCreate(itemType==="category"?{name:newName,type:newType}:{name:newName});setNewName("");setNewType("expense")}}>+ Adicionar</button>
        </div>
            ...
            <div key={item.id} className="manage-item">
              {editId===item.id?(
                <div style={{display:"flex",gap:6,flex:1,marginRight:6}}>
                  ...
                  {itemType==="category"&&<select value={editType} onChange={e=>setEditType(e.target.value)} style={{flex:1,maxWidth:120}}><option value="expense">Despesa</option><option value="income">Receita</option></select>}
                  <button className="btn bp bsm" onClick={()=>{onUpdate(item.id,itemType==="category"?{name:editName,type:editType}:{name:editName});setEditId(null)}}>✓</button>
                  <button className="btn bs bsm" onClick={()=>setEditId(null)}>✕</button>
                </div>
              ):(
                <>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <span style={{fontWeight:500,fontSize:13}}>{item.name}</span>
                    {itemType==="category"&&<span className={item.type==="income"?"mi-inc":"mi-exp"}>{item.type==="income"?"receita":"despesa"}</span>}
                  </div>
...
// ─── PAY RECURRING MODAL ─────────────────────────────────────────────────────
function PayRecurringModal({ item, accounts, year, month, onPay, onClose }: {
  item: RecurringMonthItem; accounts: any[]; year: number; month: number; onPay:(d:any)=>void; onClose:()=>void
}) {
  ...
  return (
    <div className="mbg" onClick={e=>{if(e.target===e.currentTarget)onClose()}}>
      <div className="modal">
        <div className="mt">Pagar — {item.icon} {item.name}</div>
        <div style={{marginBottom:12}}>
          <span className="fl">Valor pago {item.is_variable?"(variável — informe o valor real)":"(pode ter juros/diferença)"}</span>
        ...
        <div className="mf">
          ...
          <button className="btn bp" onClick={()=>{
            if(!accId){alert("Selecione a conta");return}
            onPay({year,month,amount:Number(payAmt),account_id:Number(accId)})
...
export default function App() {
  ...
  const PeriodNav = () => (
    <div className="period-row">
      <div className="pnav">
        <button className="parr" onClick={prevPeriod}>‹</button>
        <span className="plbl">{periodLabel}</span>
        <button className="parr" onClick={nextPeriod}>›</button>
      </div>
      <div className="vtog">
        <button className={`vtbtn ${viewMode==="month"?"on":""}`} onClick={()=>setViewMode("month")}>Mensal</button>
        <button className={`vtbtn ${viewMode==="year"?"on":""}`} onClick={()=>setViewMode("year")}>Anual</button>
      </div>
      {viewMode==="year"&&MONTHS_PT.map((m,i)=>(
        <button key={i} style={{background:"var(--s2)",border:"1px solid var(--b2)",borderRadius:8,color:"var(--muted2)",cursor:"pointer",fontSize:11,fontFamily:"'Plus Jakarta Sans',sans-serif",padding:"4px 9px"}}
    ...
    <>
      ...
      <div className="app">
          ...
          <>
            ...
            <div className="g2">
              ...
              <div className="panel">
                ...
                {catSummary.length>0?(
                  <PieChart width={265} height={260}>
                    <Pie data={catSummary} dataKey="total" nameKey="category" cx="50%" cy="50%" outerRadius={98} innerRadius={46}>
                ...
                {accSummary.map((a:any)=>(
                  <div key={a.name} className="acc-bar-item">
                    ...
                    <div style={{display:"flex",gap:14}}>
                      <span style={{fontSize:11,color:"var(--muted2)"}}>↑ {fmt(a.income)}</span>
                      <span style={{fontSize:11,color:"var(--muted2)"}}>↓ {fmt(a.expense)}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* CONTAS FIXAS DO MÊS no dashboard */}
            {recurringMonth.length>0&&viewMode==="month"&&(
              <div className="panel" style={{marginBottom:16}}>
                <div className="ph">
                  <div><div className="pt">Contas fixas — {periodLabel}</div><div className="ps">clique para marcar como pago</div></div>
                  <span className="bc">{recurringMonth.filter(r=>r.paid).length}/{recurringMonth.length} pagas</span>
                </div>
                {recurringMonth.map(item=>(
                  <div key={item.id} className={`rec-pay-row ${item.paid?"paid-border":""}`}>
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
                      <span style={{fontSize:20}}>{item.icon}</span>
                      <div>
                        <div style={{fontWeight:600,fontSize:13}}>{item.name}</div>
                        <div style={{fontSize:11,color:"var(--muted2)"}}>
                          {item.due_day?`vence dia ${item.due_day}`:"variável"}
                          {item.paid&&item.paid_amount&&<span style={{marginLeft:8,color:"var(--green)"}}>pago: {fmt(item.paid_amount)}</span>}
                        </div>
                      </div>
                    </div>
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
                      <span style={{fontFamily:"'Bricolage Grotesque',sans-serif",fontSize:14,fontWeight:700,color:item.paid?"var(--green)":"var(--red)"}}>
                        {fmt(item.paid?item.paid_amount??item.amount:item.amount)}
                      </span>
                      {item.paid?(
                        <button className="badge b-ok" style={{cursor:"pointer",border:"none"}}
                          onClick={async()=>{await unpayRecurring(item.id,{year,month});loadAll()}}>
                          ✅ pago
                        </button>
                      ):(
                        <button className="btn bp" style={{padding:"5px 12px",fontSize:12}}
                          onClick={()=>setPayingRec(item)}>
                          Pagar
                        </button>
                      )}
                    </div>
                      ...
                      <td>
                        {editAmtId===t.id?(
                          <div className="amt-edit">
                            <input className="amt-input" type="number" value={editAmtVal} onChange={e=>setEditAmtVal(e.target.value)} autoFocus />
                            <button className="btn bp bsm" onClick={async()=>{await updateTransactionAmount(t.id,Number(editAmtVal));setEditAmtId(null);loadAll()}}>✓</button>
                            <button className="btn bs bsm" onClick={()=>setEditAmtId(null)}>✕</button>
                          </div>
                        ):(
                          <div style={{display:"flex",alignItems:"center",gap:5}}>
                            <span className="badge b-e">{fmt(t.amount)}</span>
                            <button className="be" title="Editar valor (ex: juros)" onClick={()=>{setEditAmtId(t.id);setEditAmtVal(String(t.amount))}}>✏️</button>
                          </div>
                        )}
                      </td>
          ...
          <>
              ...
              <div className={`cbox ${showDebt?"on":""}`}>
                <div className="cbody" style={{flexDirection:"column"}}>
                  <div style={{width:"100%"}}>
                    ...
                    <div className="dt-btns" style={{marginTop:6}}>
                      {Object.entries(DEBT_CFG).map(([k,cfg])=><button key={k} type="button" className={`dt-btn ${debtType===k?`s-${k}`:""}`} onClick={()=>setDebtType(k)}>{cfg.icon} {cfg.label}</button>)}
                    </div>
                  </div>
                  <div style={{width:"100%",display:"flex",alignItems:"center",gap:10,background:"var(--s1)",borderRadius:10,padding:"10px 14px"}}>
                    ...
                    <label htmlFor="custom-chk" style={{fontSize:13,cursor:"pointer",fontWeight:500}}>Parcelas personalizadas (valores/datas diferentes por parcela)</label>
                  </div>
                  <form onSubmit={handleDebtSubmit} style={{width:"100%",display:"flex",flexWrap:"wrap",gap:8}}>
                    ...
                    {!customMode?(
                      <>
                        ...
                        <input style={{flex:1,minWidth:80}} type="number" placeholder="Nº parcelas" min="1" value={inst.total_installments} onChange={e=>{setInst({...inst,total_installments:e.target.value});const n=parseInt(e.target.value)||0;setCustomRows(Array.from({length:n},(_,i)=>customRows[i]||{amount:"",date:now.toISOString().slice(0,10)}))}} />
                      ...
                      <div style={{width:"100%"}}>
                        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                          ...
                          <button type="button" className="btn bp bsm" onClick={()=>setCustomRows([...customRows,{amount:"",date:now.toISOString().slice(0,10)}])}>+ Parcela</button>
                        </div>
                        ...
                        {customRows.length>0&&<div style={{fontSize:12,color:"var(--muted2)",marginTop:6}}>Total: <strong style={{color:"var(--text)"}}>{fmt(customRows.reduce((a,r)=>a+Number(r.amount||0),0))}</strong></div>}
                      </div>
                    )}
                    ...
                    {!customMode&&inst.total_amount&&inst.total_installments&&Number(inst.total_installments)>0&&(
                      <div className="hint">{DEBT_CFG[debtType].icon} {fmt(Number(inst.total_amount)/Number(inst.total_installments))}/mês · só impacta saldo quando pago</div>
                    )}
            ...
            <div className="il">
              {[...activeInst,...doneInst].map(inst=>{
                ...
                return (
                  <div key={inst.id} className={`ic ${inst.pending_installments===0?"done":""}`}>
                    <div className="it">
                      <div>
                        <span style={{background:cfg.bg,color:cfg.color,borderRadius:6,padding:"2px 7px",fontSize:10,fontWeight:600,display:"inline-block",marginBottom:4}}>{cfg.icon} {cfg.label}</span>
                        <div className="in">{inst.description}</div>
                        <div className="is">{fmt(inst.value_per_installment)}/mês</div>
                      </div>
                      <div style={{textAlign:"right"}}>
                        {inst.pending_installments>0?(
                          <>
                        ...
                        <div className="i-act" style={{justifyContent:"flex-end",marginTop:4}}>
                          {inst.pending_installments>0&&<button className="be" onClick={()=>setEditingInst(inst)}>✏️</button>}
                    ...
                    <div className="pw"><div className="pf" style={{width:`${inst.progress_percent}%`,background:inst.pending_installments===0?"var(--green)":`linear-gradient(90deg,${cfg.color},${cfg.color}99)`}} /></div>
                    <div className="ib">
                      ...
                      {inst.pending_installments===0&&<div className="idone">✅ Quitado</div>}
                    </div>
                  </div>
                )
              })}
              {filteredInst.length===0&&<div style={{color:"var(--muted2)",fontSize:13,textAlign:"center",padding:"28px 0"}}>Nenhuma dívida{debtFilter!=="todos"?` do tipo selecionado`:""}.</div>}
            </div>
          </>
        )}
        ...
        {page==="fixed"&&(
          <>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
              <div><div style={{fontFamily:"'Bricolage Grotesque',sans-serif",fontSize:20,fontWeight:700}}>Contas fixas</div><div style={{fontSize:12,color:"var(--muted2)",marginTop:2}}>Recorrências mensais — fixas e variáveis</div></div>
              <button className="btn bp" onClick={()=>{setEditingRec(null);setShowRecModal(true)}}>+ Nova conta</button>
            </div>

            {recurringItems.length>0&&(
              <div className="chips">
                <div className="chip"><div className="chip-lbl">Total mensal estimado</div><div className="chip-val" style={{color:"var(--red)"}}>{fmt(recTotal)}</div></div>
            ...
            {recurringItems.length===0?(
              <div className="panel" style={{textAlign:"center",padding:"40px 0"}}>
                ...
                <div style={{color:"var(--muted2)",fontSize:13}}>Nenhuma conta fixa cadastrada.</div>
              </div>
            ):(
              <div className="rl">
                {recurringItems.filter(r=>!r.is_variable).length>0&&<div style={{fontSize:11,fontWeight:700,color:"var(--muted2)",textTransform:"uppercase",letterSpacing:".7px",margin:"8px 0 4px"}}>Com vencimento fixo</div>}
                  ...
                  <div key={item.id} className={`ri ${!item.active?"inactive":""}`}>
                    <div className="ri-left">
                      ...
                      <div>
                        ...
                        <div className="ri-day">dia {item.due_day}{!item.active&&<span style={{marginLeft:6,fontSize:10,color:"var(--muted)"}}>inativa</span>}</div>
                      </div>
                    </div>
                ...
                {recurringItems.filter(r=>r.is_variable).length>0&&<div style={{fontSize:11,fontWeight:700,color:"var(--muted2)",textTransform:"uppercase",letterSpacing:".7px",margin:"12px 0 4px"}}>Variáveis</div>}
                {recurringItems.filter(r=>r.is_variable).map(item=>(
                  <div key={item.id} className={`ri ${!item.active?"inactive":""}`}>
                    <div className="ri-left">
                      ...
                      <div>
                        <div className="ri-name">{item.name}</div>
                        <div className="ri-day">variável{!item.active&&<span style={{marginLeft:6,fontSize:10,color:"var(--muted)"}}>inativa</span>}</div>
                      </div>
                    </div>
                    <div className="ri-right">
                      {editingVarId===item.id?(
                        <div style={{display:"flex",gap:6,alignItems:"center"}}>
                          <input type="number" value={varAmt} onChange={e=>setVarAmt(e.target.value)} style={{width:100,padding:"5px 8px",fontSize:13}} autoFocus />
                          <button className="btn bp bsm" onClick={async()=>{await updateRecurringAmount(item.id,Number(varAmt));setEditingVarId(null);loadAll()}}>✓</button>
                          <button className="btn bs bsm" onClick={()=>setEditingVarId(null)}>✕</button>
                        </div>
                      ):(
                        <>
                          <div className="ri-amt" style={{color:"var(--yellow)"}}>{fmt(item.amount)}</div>
                          <div className="ri-act">
                            <button className="be" title="Atualizar valor" onClick={()=>{setEditingVarId(item.id);setVarAmt(String(item.amount))}}>💰</button>
                            <button className="be" onClick={()=>{setEditingRec(item);setShowRecModal(true)}}>✏️</button>
                            <button className="bd" onClick={async()=>{if(confirm("Remover?"))await deleteRecurring(item.id);loadAll()}}>🗑</button>
                          </div>
                        </>
        ...
        {page==="settings"&&(
          <div className="g2b">
            <div className="panel">
              <div className="ph"><div><div className="pt">Categorias</div><div className="ps">gerencie suas categorias</div></div><button className="btn bp bsm" onClick={()=>setShowCatModal(true)}>Gerenciar</button></div>
              <div style={{maxHeight:300,overflowY:"auto"}}>
                {categories.map(c=><div key={c.id} className="manage-item"><div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontWeight:500,fontSize:13}}>{c.name}</span><span className={c.type==="income"?"mi-inc":"mi-exp"}>{c.type==="income"?"receita":"despesa"}</span></div></div>)}
              </div>
            </div>
            <div className="panel">
              <div className="ph"><div><div className="pt">Contas & Cartões</div><div className="ps">gerencie suas contas</div></div><button className="btn bp bsm" onClick={()=>setShowAccModal(true)}>Gerenciar</button></div>
              <div style={{maxHeight:300,overflowY:"auto"}}>
                {accounts.map(a=><div key={a.id} className="manage-item"><span style={{fontWeight:500,fontSize:13}}>🏦 {a.name}</span></div>)}
              </div>
            </div>
          </div>
        )}
